<?php

namespace App\Services;

use App\Helpers\ApiResponse;
use App\Repositories\AccessFeatureRepository;

class AccessFeatureService
{
    protected $repo;

    public function __construct(AccessFeatureRepository $repo)
    {
        $this->repo = $repo;
    }
    public function getAccessFeature()
    {
       return ApiResponse::success($this->repo->show(), 'Successfully', 201);
    }
    public function createAccessFeature($data)
    {
       $response = $this->repo->create($data['features'], $data['roles']);
       return ApiResponse::success($response, 'Create Successfully', 201);
    }
    public function updateAccessFeature($id, $data)
    {
       $response = $this->repo->update($id, $data);
       return ApiResponse::success($response, 'Update Successfully', 201);
    }
    public function deleteAccessFeature($id)
    {
       $response = $this->repo->delete($id);
       return ApiResponse::success($response, 'Delete Successfully', 201);
    }
}